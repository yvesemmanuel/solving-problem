...
Proof.
  ...
  - simpl. rewrite <- mult_n_Sm. rewrite -> plus_comm. rewrite -> IHm'.  reflexivity. Qed.
(** [] *)
...
Theorem andb_false_r : forall b : bool,
  andb b false = false.
Proof.
  intros b. induction b as [|]. reflexivity. reflexivity. Qed.

(** the whole is bigger than its parts**)
Theorem whole_bigger : forall n m : nat,
  n <=? n + m = true.
Proof.
  intros n m. induction n as [| n' IHn'].
  - simpl. reflexivity.
...
Theorem equiv_comp : forall p n m : nat,
  (p + n <=? p + m) = (n <=? m).
Proof.
  intros p n m. induction p as [| n' IHn'].
  - simpl. reflexivity.
...
Proof.
  intros n m p. intros H. induction n as [|]. 
  - rewrite <- plus_n_O. rewrite -> whole_bigger. reflexivity.
  - rewrite -> equiv_comp. rewrite -> H. reflexivity. Qed.
...
Theorem exclude_middle : forall a : bool,
  a || negb a = true.
Proof.
  intros a. destruct a as [|] eqn:Ea.
  - simpl. reflexivity.
  - simpl. reflexivity. Qed.
...
Theorem all3_spec : forall b c : bool,
  ...
  = true.
Proof.
  intros b c. induction b as [|].
  - simpl. rewrite -> exclude_middle. reflexivity.
  - simpl. reflexivity. Qed.
