...
(*implication is only true if 'andb b c = true' is false,
when 'andb b c = true' is true,
c = true is too. *)
...
Proof.
  (*I should assume (forall x : bool, f x = x) is true for all f : bool -> bool*)
  intros f. intros H. intros b0.
  rewrite -> H.
  rewrite -> H.
  reflexivity. Qed.
...
Inductive bin : Type :=
  | Z (*zero*)
  | B0 (n : bin) (* all even numbers begins with B0 -> 2n *)
  | B1 (n : bin). (* all odd numbers begins with B1 -> 2n + 1 *)
...
Fixpoint incr (m:bin) : bin :=
  match m with
  | Z => B1 Z (* increment ZERO => 2n + 1, n = 0 *)
  | B0 m' => B1 m' (* increment EVEN => 2n + 1 *)
  | B1 m' => B0 (incr m') (* increment ODD => (2n + 1) + 1 = 2(n + 1) *)
  end.

Fixpoint bin_to_nat (m:bin) : nat :=
  match m with
  | Z => 0 (* base case *)
  | B0 m' => mult 2 (bin_to_nat m') (* if m is an even number *)
  | B1 m' => plus 1 (mult 2 (bin_to_nat m')) (* if m is an odd number *)
  end.
