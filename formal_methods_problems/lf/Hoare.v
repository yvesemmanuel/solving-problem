...
Theorem hoare_pre_false : forall (P Q : Assertion) c,
  ...
  {{P}} c {{Q}}.
Proof.
  intros P Q c H st st' H' H''.
  apply H in H''. (* get a contradiction from the fact that P st is false *)
  contradiction.
Qed.
...
Example hoare_asgn_examples2 :
  exists P,
    ...
    {{ 0 <=  X /\ X <= 5 }}.
Proof.
  exists (fun st => True). (* set the precondition to P = (fun st => True) *)
  unfold hoare_triple. (* use the definition of Hoare triple *)
  intros st st' HX HP. (* assume the preconditions hold *)
  simpl in HX.
Admitted.
(** [] *)
...
Proof.
  ...
  intros a P. unfold hoare_triple. intros st st' E HP.
Admitted.
...
Proof.
  unfold hoare_triple.
  intros H.
  specialize H with (a := (ANum 0)) (n := 1).
Admitted.
(** [] *)
...
Proof.
  ...
  - eapply hoare_consequence_pre. apply hoare_asgn.
    ...
    apply Nat.leb_le in H0. assumption.
  - eapply hoare_consequence_pre. apply hoare_asgn.
    unfold bassn, assn_sub, update, assert_implies.
...
Proof.
  eapply E_If1True. simpl. reflexivity.
...
Proof.
  eapply E_If1False. simpl. reflexivity.
Qed.
...
Theorem hoare_if1 : forall P Q b c,
  {{ P }} c {{ Q }} ->
  {{ fun st => P st /\ bassn b st }} (if1 b then c end) {{ Q }}.
Proof.
  intros. intros st st' Hp. intros H'.
