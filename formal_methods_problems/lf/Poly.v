...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
  intros A. intros l m n. induction l as [| p l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite <- IHl'. reflexivity. Qed.
...
Proof.
  intros X. intros l1 l2. induction l1 as [| p l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite -> IHl'. reflexivity. Qed.
(** [] *)
...
Theorem append_nil: forall X (l : list X),
  l ++ [ ] = l.
Proof.
  intros. induction l as [| l n IHl].
  - simpl. reflexivity.
  - simpl. rewrite -> IHl. reflexivity.
  Qed.

Theorem append_association: forall X (l0 l1 l2: list X),
  (l0 ++ l1) ++ l2 = l0 ++ l1 ++ l2.
Proof.
  intros. induction l0 as [| l0 n IHl0].
  - simpl. reflexivity.
...
Proof.
  intros. induction l1 as [| l1 n IHl1].
  - simpl. rewrite -> append_nil. reflexivity.
  - simpl. rewrite -> IHl1. rewrite -> append_association. reflexivity. Qed.
...
(* 
  Must resolve -> map f (rev l' ++ [n]) = rev (map f l') ++ map f [n]
*)

Lemma commutative_map : forall (X Y : Type) (f : X -> Y) (l l' : list X),
...
Example test_fold_length1 : fold_length [4;7;0;1] = 4.
Proof. reflexivity. Qed.
